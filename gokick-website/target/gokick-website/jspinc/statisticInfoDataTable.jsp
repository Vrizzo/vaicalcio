...
<ResultSet xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="urn:yahoo:lcl" xsi:schemaLocation="urn:yahoo:lcl http://local.yahooapis.com/LocalSearchService/V2/LocalSearchResponse.xsd"
  ...
  >
  <c:set var="deleteMessage"><u:translation key="message.squadRimuoviGK" /></c:set>
<s:iterator value="dataTable.results" var="result" >
  <Result>
    <Empty><![CDATA[ &nbsp; ]]></Empty>
    <EmptySquad><![CDATA[ &nbsp; ]]></EmptySquad>
    <Id><![CDATA[<s:checkbox name="userCheckList" fieldValue="%{#result.userInfo.id}" cssClass="chkSelectUser"/>]]></Id>
    <Friend><![CDATA[<s:url action="viewAvatar" var="imageUrl"><s:param name="idUser">${result.userInfo.id}</s:param></s:url><img src="${imageUrl}" alt="" />]]></Friend>
    ...
    <CompleteName><![CDATA[ <u:printUserName userInfo="${result.userInfo}" linkPlayMorePartnerStatus="${linkPlayMorePartnerStatus}" />]]></CompleteName>
    ...
    <Condition><![CDATA[<u:printUserPhysicalCondition userToShow="${result.userInfo}" />]]></Condition>
    <Played><![CDATA[${result.allTot}]]></Played>
    <Frequency><![CDATA[${result.monthPlayed}]]></Frequency>
    <Goal><![CDATA[${result.goalsTot}]]></Goal>
    <AvgGoal><![CDATA[${result.averageGoal}]]></AvgGoal>
    <AvgVote><![CDATA[${result.averageVote}]]></AvgVote>
    ...
    <NationalitySquad>
      <![CDATA[
      <img src="<s:url value="/images/flags/"/>${result.userInfo.idNatCountry}.png" title="${result.userInfo.natCountry}" onError="this.src='<s:url value="/images/"/>country_flag_0.gif';" />
      ]]>
    </NationalitySquad>
    <RoleSquad><![CDATA[<c:set var="ruoloLang" ><u:translation key="${result.userInfo.playerRoleKey}"/></c:set>${fn:substring(ruoloLang, 0, 1)}]]></RoleSquad>
    <AgeSquad><![CDATA[${result.userInfo.age}]]></AgeSquad>
    <PlayedSquad><![CDATA[${result.allTot}]]></PlayedSquad>
    <ReliabilitySquad><![CDATA[${result.reliability}%]]></ReliabilitySquad>
    <Remove><![CDATA[
      <c:if test="${result.userInfo.id != currentUser.id}">
        <s:url   action="squadRemoveUser" var="removeUser" namespace="" >
          <s:param name="idUser">${result.userInfo.id}</s:param>
        </s:url>
          <a    class="${currentUserClass} delete"  onclick="return confirm('${deleteMessage}');" href="${removeUser}">X</a>
       </c:if>]]>
    </Remove>
  </Result>
</s:iterator>
