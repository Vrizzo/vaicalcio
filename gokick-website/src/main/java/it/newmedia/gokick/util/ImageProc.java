...
//            return outputStream.toByteArray();
  ...
  public static BufferedImage resize(BufferedImage inputImage, int newWidth, int newHeight, boolean lockAspectRatio, boolean enlargeIfSmaller) throws Exception
  {
    ...
    if(inputImage.getType()==0 || inputImage.getType()==6)
    {
  ...
  {
    return ImageProc.cropByDimensions(inputImage, xStart, yStart, xEnd - xStart, yEnd - yStart);
  }
          ...
          ) throws Exception
  {
    ...
    if (expandTop != 0 ||
            ...
            expandLeft != 0)
    {
      res = ImageProc.addBorders(res, backImageBgFill, expandTop, expandRight, expandBottom, expandLeft);
    }
